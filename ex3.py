...
# The code has been written for each particular case instead of having one big code covering all bases
...
# Perform left rotation on a node (example):
# root._left_rotate(node_to_rotate)
...
# Perform right rotation on a node (example):
# root._right_rotate(node_to_rotate)
...
# Here is a new single code in case the separate codes are not sufficient along with the test cases
class Node:
    def __init__(self, data, parent=None, left=None, right=None, balance_factor=0):
        self.parent = parent
        self.data = data
